#!/usr/bin/env python3
"""
cmd_vel_to_sport_demo.py  --  Go2 ClassicWalk(AI sport) cmd_vel 브리지 (정리판)

이 파일은 scripts/ 안의 cmd_vel_to_sport_v0~v6 실험본을 하나로 정리한
"사용 가능한 표준본"이다. Nav2(또는 teleop) 의 /cmd_vel 을 받아
Unitree Go2 sport-mode Request 로 변환해 /api/sport/request 로 보낸다.

핵심 포인트 (왜 이렇게 했나):
  ClassicWalk(2049) 는 FreeWalk/FreeBound/... (2045~2051) 와 함께
  Go2 의 "AI(고급) sport" 서비스 전용 gait 다. 기본(normal) sport 모드에서
  /api/sport/request 로 2049 를 보내면 무시된다.
  -> 따라서 부팅 시 motion_switcher 로 먼저 모드를 'ai' 로 바꾼 뒤
     ClassicWalk 를 켠다.
...
사용법:
  ...
  # 파라미터 예:
  python3 cmd_vel_to_sport_demo.py --ros-args -p speed_level:=fast -p select_ai_mode:=true
"""
import json
import math
import time
...
class CmdVelToSportDemo(Node):
    def __init__(self):
        super().__init__('cmd_vel_to_sport_demo')
        ...
        L = self.get_logger()
        L.info('=' * 60)
        L.info('   cmd_vel_to_sport_demo (ClassicWalk@AI)  --  PARAMETER SUMMARY')
        L.info('=' * 60)
