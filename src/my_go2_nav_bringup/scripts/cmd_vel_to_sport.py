...
# --- 보행 모드(gait) API ID  (unitree_sdk2_python/go2/sport/sport_api.py 기준) ---
SPORT_API_CLASSICWALK = 2049   # ClassicWalk(flag): 기본(클래식) 보행, 파라미터 {"data": flag}
...
class CmdVelToSport(Node):
    def __init__(self):
        ...
        self.declare_parameter('set_classic_walk', True)   # ClassicWalk(기본 보행) on/off
        self.declare_parameter('classic_walk_flag', True)  # ClassicWalk 에 보낼 flag (True=enable)
        ...
        L = self.get_logger()
        L.info('=' * 60)
        L.info('   cmd_vel_to_sport (v3)  --  PARAMETER SUMMARY')
        L.info('=' * 60)
        ...
        L.info(f'  init: balance={self.init_balance_stand} wait={self.init_wait_sec}s '
               ...
               f'classic_walk={self.set_classic_walk}(flag={self.classic_walk_flag})')
    ...
    def send_classic_walk(self):
        # SDK 의 ClassicWalk(flag) 와 동일: api_id 2049, parameter {"data": flag}
        self.pub.publish(self._make_request(
            SPORT_API_CLASSICWALK, json.dumps({'data': self.classic_walk_flag})))
        self.get_logger().info(
            f'[INIT] ClassicWalk gait set (flag={self.classic_walk_flag})')
