#!/usr/bin/env python3
"""
cmd_vel_to_sport (v5)  --  ClassicWalk via AI sport mode

왜 v2~v4 의 ClassicWalk 가 안 먹혔나:
  ClassicWalk(2049) 는 FreeWalk/FreeBound/FreeJump/FreeAvoid/WalkUpright/CrossStep
  (2045~2051) 와 함께 Go2 의 "AI(고급) sport" 서비스 전용 gait 다.
  기본(normal) sport 모드에서 /api/sport/request 로 2049 를 보내면 무시된다.

해결:
  먼저 motion_switcher 로 모드를 'ai' 로 바꾼 뒤 ClassicWalk 를 보낸다.
    - /api/motion_switcher/request  (unitree_api/msg/Request)
      SelectMode : api_id=1002, parameter={"name": "<mode>"}     (기본 "ai")
      (필요시) ReleaseMode : api_id=1003
    - /api/sport/request
...
API id / parameter 규약은 unitree_ros2 공식 예제
  example/src/src/common/ros2_sport_client.cpp
  example/src/include/g1/g1_motion_switch_client.hpp
를 따른다.
"""
import json
import math
...
# --- motion_switcher service (g1_motion_switch_client.hpp 의 상수) ---
...
class CmdVelToSport(Node):
    def __init__(self):
        ...
        L = self.get_logger()
        L.info('=' * 60)
        L.info('   cmd_vel_to_sport (v5 / ClassicWalk@AI)  --  PARAMETER SUMMARY')
        L.info('=' * 60)
