...
SPORT_API_STATICWALK   = 2049   # StaticWalk 안정 보행
...
class CmdVelToSport(Node):
    def __init__(self):
        ...
        L.info('  init seq  : BalanceStand(1002) -> SpeedLevel(1015/normal) -> StaticWalk(1061)')
        L.info('=' * 60)
    ...
    def send_static_walk(self):
        self.pub.publish(self._make_request(SPORT_API_STATICWALK, noreply=False))
        self.get_logger().info('[INIT] Sending StaticWalk (1061)...')
    ...
    def do_init_sequence(self):
        """
        BalanceStand(1002) → init_wait_sec → SpeedLevel(1015) → StaticWalk(1061) → ready
        """
